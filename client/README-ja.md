...
|-------------|-----------------|------|
| [nostter](evidences/bootstrap-relay/nostter.md) | nos.lol, relay.damus.io (環境変数 VITE_DEFAULT_RELAYS で上書き可) | 前回調査時にあった relay.nostr.band がデフォルトから削除され、現在は nos.lol と relay.damus.io の2件のみ。日本語ロケール時のみ web/src/routes/+layout.ts の addDefaultRelays(localizedRelays.ja) で日本リレーを追加する。 |
| [Rabbit](evidences/bootstrap-relay/rabbit.md) | relay.damus.io, nos.lol, relay.snort.social, relay.nostr.wirednet.jp | ブラウザ言語が 'ja' の場合に日本語リレー追加 (relay-jp.nostr.wirednet.jp, r.kojira.io, yabu.me)。判定は src/core/useConfig.ts:129-135 の initialRelays()。旧版にあった nostr.holybea.com は削除された。 |
| [Lumilumi](evidences/bootstrap-relay/lumilumi.md) | directory.yabu.me, purplepag.es, indexer.coracle.social, nostr.wine | relaySearchRelays は kind:0/3/10002 取得用ブートストラップ、defaultRelays は kind:10002 未検出時のフォールバック (nos.lol, nostr.bitcoiner.social, nostr-pub.wellorder.net, relay.snort.social)。旧調査から relaySearchRelays に nostr.wine 追加・relay.nostr.band 除外。 |
...
| [野雨](evidences/bootstrap-relay/nosame.md) | relay-jp.nostr.wirednet.jp, yabu.me, r.kojira.io, nostr.compile-error.net | app.js が config.js 等に分割された。リレー一覧が変更され relay.barine.co が削除、nostr.compile-error.net が追加された。ロケール分岐はなく常に日本リレー固定。 |
...
| [Yakihonne](evidences/bootstrap-relay/yakihonne.md) | relaysOnPlatform 6リレー (NDK explicitRelayUrls, enableOutboxModel: true) | src/Helpers/NDKInstance.js (行6-14) で初期化。リレー一覧が更新され nos.lol が追加、旧 relay.nostr.band は削除。yakihonne 自身のリレー (nostr-01/02) を含む。 |
| [iris](evidences/bootstrap-relay/iris.md) | temp.iris.to, vault.iris.to, relay.damus.io, relay.snort.social, relay.primal.net | 前回調査から変更: nos.lol が削除され relay.primal.net が追加された。環境変数 VITE_USE_TEST_RELAY / VITE_USE_LOCAL_RELAY でテスト用・ローカルに切替可能。 |
...
| [Coracle](evidences/bootstrap-relay/coracle.md) | DEFAULT_RELAYS: relay.damus.io, nos.lol / INDEXER_RELAYS: relay.damus.io, purplepag.es, indexer.coracle.social | .env.template で定義。前回調査の relay.nostr.band は INDEXER から外れ DEFAULT に damus 追加。初期化時に DEFAULT/DVM/INDEXER/SEARCH 全リレーへ接続。 |
| [noStrudel](evidences/bootstrap-relay/nostrudel.md) | DEFAULT_FALLBACK_RELAYS: relay.primal.net, relay.damus.io / lookup: purplepag.es, index.hzrd149.com, indexer.coracle.social | RECOMMENDED_FALLBACK_RELAYS は +nos.lol。lookup (プロフィール・outbox 取得) リレーに indexer.coracle.social が追加されている。 |
| [Amethyst](evidences/bootstrap-relay/amethyst.md) | bootstrapInbox 7リレー (nos.lol, nostr.mom, relay.primal.net, relay.damus.io, nostr.bitcoiner.social, nostr.oxtr.dev, directory.yabu.me) | Constants.kt が amethyst/model から commons/defaults へ移動。旧 bootstrapInbox の relay.nostr.band が外れ directory.yabu.me が追加。消費は Nip65RelayListState.normalizeNIP65ReadRelayListWithBackup() (63行)。 |
